...
func (bs *BlockScanner) SaveLocalBlock(blockHeader *pb.InternalBlock) error {
	...
	header := &openwallet.BlockHeader{
		Hash:              hex.EncodeToString(blockHeader.Blockid),
		Previousblockhash: hex.EncodeToString(blockHeader.PreHash),
...
func (bs *BlockScanner) GetLocalBlock(height uint64) (*pb.InternalBlock, error) {
	...
	blockHash, _ := hex.DecodeString(header.Hash)
	preHash, _ := hex.DecodeString(header.Previousblockhash)
